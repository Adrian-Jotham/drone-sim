...
###########################################################################
# Quadrotor Hover Training — PPO | SAC | TD3
#
# Supports standard waypoint hover and disturbance-rejection training.
# Use --wind_scale > 0 to inject OU wind disturbances during training.
#
# All algorithms log to ./hover_logs/<algo>/ for TensorBoard comparison:
#   tensorboard --logdir hover_logs
#
# Usage:
#   python train_hover.py --algo ppo
#   python train_hover.py --algo sac --wind_scale 0.8
#   python train_hover.py --algo td3 --obs_noise
#   python train_hover.py --algo sac --headless --total_timesteps 3000000
###########################################################################
...
from disturbance.quadrotor_hover_env import (
    ...
    _DRONE_MESH_NAME, _load_drone_mesh,
    WIND_MAX,
)
    ...
    def __init__(self, viewer, train_vec_env, render_freq: int = 5_000, verbose: int = 0):
        ...
        if viewer is not None:
            ...
            if mesh is not None:
                ...
                viewer.log_mesh(_DRONE_MESH_NAME, points, indices, normals=normals)
...
class MetricsCallback(BaseCallback):
    ...
    def __init__(self, log_freq: int = 1_000, window: int = 100, verbose: int = 0):
        ...
        self._wind_mags  = deque(maxlen=window)
    ...
    def _on_step(self) -> bool:
        for done, info in zip(self.locals.get("dones", []), self.locals.get("infos", [])):
            ...
            wm = info.get("wind_magnitude", 0.0)
            if wm > 0:
                self._wind_mags.append(wm)
    ...
    def _flush(self) -> None:
        ...
        if self._wind_mags:
            rec("metrics/mean_wind_N",  np.mean(self._wind_mags))
...
def main() -> None:
    ...
    parser.add_argument("--wind_scale",      type=float, default=0.0,
                        help="OU wind disturbance strength 0=off 1=full "
                             f"(max ~{WIND_MAX:.1f} N). Trains disturbance rejection.")
    ...
    wind_str = f"{args.wind_scale:.2f} (max {WIND_MAX:.1f} N)" if args.wind_scale > 0 else "off"
    print(
        ...
        f"  TensorBoard  → tensorboard --logdir hover_logs\n"
    )
